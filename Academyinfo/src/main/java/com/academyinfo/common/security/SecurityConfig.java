...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
            ...
            .and() // POST 사용 시 security 무시해야 하는 영역
    			//다른 부분은 무시해주시고 이 부분만 잘 사용 해주시면 됩니다.
    			.csrf()
    			.ignoringAntMatchers("/freeboard/post")
    			.ignoringAntMatchers("/freeboard/post/{no}/comments")
    			.ignoringAntMatchers("/freeboard/post/{no}/comments/{writer}")
    			.ignoringAntMatchers("/alertboard/post")
    			.ignoringAntMatchers("/user/update")
    		    	 //
            .and() // 로그아웃 설정
                .logout()
